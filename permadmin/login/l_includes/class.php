...
if(!class_exists('shutt_class')){
	class shutt_class {
		...
		function register($redirect) {
			...
			if ( !empty ( $_POST ) ) {
				...
				if ( $referrer == $current ) {
					...
					$fields = array('user_name', 'user_login', 'user_pw', 'user_email', 'user_reg');

					//These are the values from our registration form... cleaned using our clean method
					$values = $ldb->clean($_POST);
					...
					$insert = $ldb->insert($link, $table, $fields, $values);
		...
		function login($redirect) {
			...
			if ( !empty ( $_POST ) ) {
				...
				if ( $subpass == $stopass ) {
					...
					setcookie('shuttlogauth[user]', $subname, 0, '', '', '', true);
					setcookie('shuttlogauth[authID]', $authID, 0, '', '', '', true);
		...
		function logout() {
			...
			$idout = setcookie('shuttlogauth[authID]', '', -3600, '', '', '', true);
			$userout = setcookie('shuttlogauth[user]', '', -3600, '', '', '', true);
